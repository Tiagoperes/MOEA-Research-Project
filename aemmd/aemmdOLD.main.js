(function () {
  ...
  function createDominationTable(objectives, maxLength) {
    return {
  ...
  function createDominationTables(objectives, maxLength) {
    var tables = [];
    for (let i = 2; i <= objectives.length; i++) {
      tables = _.concat(tables, _.map(getCombinations(objectives, i), _.partial(createDominationTable, _, maxLength)));
    }
    return tables;
  }
  ...
  function aemmd(settings) {
    var tables = createDominationTables(settings.objectives, settings.elementsPerTable),
        population = generateRandomPopulation(settings.elementsPerTable * tables.length, settings.randomize),
